...
export const useWeekKPIData = (weekDate: Date) => {
  ...
  const fetchWeekData = useCallback(async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const calls = dayEntries.find((e: any) => e.kpi_type === 'calls')?.value || 0;
      const appraisals = dayEntries.find((e: any) => e.kpi_type === 'appraisals')?.value || 0;
      const openHomes = dayEntries.find((e: any) => e.kpi_type === 'open_homes')?.value || 0;
      ...
      const logged = dayEntries.length > 0 && dayEntries.some((e: any) => e.value > 0);
