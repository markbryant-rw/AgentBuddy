...
export const BeaconTab = ({ appraisal }: BeaconTabProps) => {
  ...
  const lastActivity = appraisal.beacon_last_activity;
  const firstViewedAt = appraisal.beacon_first_viewed_at;
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center justify-between">
            ...
            <div className={cn(
              "flex-1 h-0.5 mx-2",
              appraisal.beacon_synced_at ? "bg-teal-500" : "bg-muted"
            )} />
            ...
            <div className={cn(
              "flex-1 h-0.5 mx-2",
              totalViews > 0 ? "bg-teal-500" : "bg-muted"
            )} />
            ...
            <div className="flex flex-col items-center">
              ...
              {totalViews > 0 ? (
                <span className="text-xs text-teal-600 font-medium mt-1">{totalViews}×</span>
              ) : (
            ...
            <div className="flex flex-col items-center">
              <div className={cn(
                ...
                <Flame className={cn("h-5 w-5", isHotLead && "animate-pulse")} />
              </div>
              <span className="text-xs font-medium">Hot Lead</span>
              {isHotLead ? (
                <Check className="h-3.5 w-3.5 text-red-600 mt-1" />
              ) : (
                <span className="text-[10px] text-muted-foreground mt-1">70+ score</span>
              )}
            </div>
          </div>
        </CardContent>
          ...
          <CardContent className="space-y-2">
            {firstViewedAt && (
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className="font-medium">{format(new Date(firstViewedAt), 'MMM d, yyyy h:mm a')}</span>
              </div>
            )}
            {lastActivity && (
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className="font-medium">{format(new Date(lastActivity), 'MMM d, yyyy h:mm a')}</span>
              </div>
            )}
          </CardContent>
        </Card>
