...
serve(async (req) => {
  ...
  try {
    ...
    if (assignedTeamId) {
      ...
      console.log('Sending Beacon member add webhook after invitation acceptance...');
      sendBeaconMemberChange(supabaseAdmin, assignedTeamId, {
        action: 'add',
        member: {
          user_id: userId,
          email: invitation.email,
          full_name: fullName,
          phone: mobileNumber || '',
          is_team_leader: invitation.role === 'team_leader',
        },
      });
