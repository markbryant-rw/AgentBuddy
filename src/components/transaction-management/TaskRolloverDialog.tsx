...
import { useTaskRollover, STAGE_DISPLAY_NAMES } from '@/hooks/useTaskRollover';
import { toast } from 'sonner';

interface TaskRolloverDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onComplete: () => void;
...
export const TaskRolloverDialog = ({
  isOpen,
  onClose,
  onComplete,
  transactionId,
  currentStage,
  targetStage,
}: TaskRolloverDialogProps) => {
  const [tasks, setTasks] = useState<TaskPreview[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [isProcessing, setIsProcessing] = useState(false);
  const [showTasks, setShowTasks] = useState(false);
  
  const { getIncompleteTasksForTransaction, rolloverTasks, clearIncompleteTasks } = useTaskRollover();

  useEffect(() => {
    if (isOpen && transactionId) {
      setIsLoading(true);
      getIncompleteTasksForTransaction(transactionId)
        .then(setTasks)
        .catch(console.error)
  ...
  const handleKeepTasks = async () => {
    ...
    try {
      const count = await rolloverTasks(transactionId, currentStage);
      toast.success(`${count} tasks moved to ${currentStage.toUpperCase()} TASKS section`);
      onComplete();
    } catch (error) {
  ...
  const handleStartFresh = async () => {
    setIsProcessing(true);
    try {
      const count = await clearIncompleteTasks(transactionId);
  ...
  const targetStageName = STAGE_DISPLAY_NAMES[targetStage] || targetStage;
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            uncompleted tasks from the <span className="font-medium">{stageName}</span> stage.
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Tasks will be moved to a "{stageName.toUpperCase()} TASKS" section 
