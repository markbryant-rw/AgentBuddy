...
-- Create comprehensive seed_demo_data function with realistic Auckland data
CREATE OR REPLACE FUNCTION seed_demo_data(p_demo_user_id uuid DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_outcomes text[] := ARRAY['PENDING', 'PENDING', 'PENDING', 'PENDING', 'WON', 'LOST']; -- Weighted toward pending
  v_agent_ids uuid[];
  v_random_agent uuid;
  v_random_idx int;
  v_price int;
...
BEGIN
  ...
  -- Agent IDs for distribution (demo user gets more, then decreasing)
  ...
  FOR v_i IN 1..80 LOOP
    -- Distribute: Demo 25, Sarah 25, Mike 18, Emma 12
    IF v_i <= 25 THEN
    ...
    -- Random price based on suburb tier
  ...
  -- ========================================
  -- ACTIVE TRANSACTIONS (12 total)
  -- ========================================
  FOR v_i IN 1..12 LOOP
    -- Distribute: Demo 4, Sarah 4, Mike 3, Emma 1
    IF v_i <= 4 THEN
      v_random_agent := v_demo_user_id;
    ELSIF v_i <= 8 THEN
      v_random_agent := v_sarah_id;
    ...
    INSERT INTO transactions (
      ...
      stage, assigned_to, team_id, created_by,
  ...
  -- ========================================
  -- PAST SALES (45 total over 2 years)
  -- ========================================
  FOR v_i IN 1..45 LOOP
    -- Distribute: Demo 15, Sarah 16, Mike 10, Emma 4
    IF v_i <= 15 THEN
      v_random_agent := v_demo_user_id;
    ELSIF v_i <= 31 THEN
      v_random_agent := v_sarah_id;
    ...
    INSERT INTO past_sales (
      address, suburb, vendor_name, sale_price,
      settlement_date, unconditional_date, listing_signed_date, listing_live_date,
      days_on_market, lead_source, status,
      lead_salesperson_id, team_id, created_by
    ) VALUES (
      v_auckland_streets[1 + floor(random() * array_length(v_auckland_streets, 1))::int],
      v_suburbs[v_random_idx],
      v_vendor_names[1 + floor(random() * array_length(v_vendor_names, 1))::int],
      v_price,
      CURRENT_DATE - (floor(random() * 730)::int), -- Past 2 years
      ...
      v_lead_sources[1 + floor(random() * array_length(v_lead_sources, 1))::int],
      CASE WHEN v_i <= 42 THEN 'WON' ELSE 'WITHDRAWN' END, -- 3 withdrawals
      v_random_agent,
      v_demo_team_id,
      v_random_agent
    );
